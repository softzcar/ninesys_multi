<template>
  <div>
    ...
    <b-modal :id="modal" title="Ordenes guardadas" hide-footer size="lg">
      <div class="mb-4">
        <b-table ref="table" responsive small :fields="fieldsGuardadas" :items="ordenesGuardadas" class="mb-4">
          <template #cell(form)="data">
            <b-button variant="link" @click="loadFormGuardadas(data.index, data.item._id)">
              {{ data.item.form.nombre }} {{ data.item.form.apellido }}
            </b-button>
            <pre>{{ data.item.form }}</pre>
          </template>
        ...
        <b-container>
          ...
          <b-row>
            <b-col>
              ...
              <!-- Guardar Presupuesto para terminarlao después -->
              <b-row>
                <b-col>
                  <b-button @click="guardarPresupuesto" class="floatme" variant="success">Guardar</b-button>
                  <b-button @click="cargarPresupuesto" class="floatme" variant="info">Cargar</b-button>
                  <b-button @click="clearForm" class="floatme" variant="danger">Limpiar formulario</b-button>
                </b-col>
              </b-row>
              <b-row>
                <b-col>
                                  ...
                                  <b-form v-on:submit.prevent>
                                    ...
                                    <b-form-group>
                                      ...
                                      <b-form-input id="input-telefono" v-model="form.telefono" type="tel"
                                        class="mb-2 mr-sm-2 mb-sm-0" placeholder="Ingrese teléfono"></b-form-input>
                                    </b-form-group>
                          ...
                          <div class="wizard-content">
                            <b-row>
                              <b-col lg="12">
                                <b-list-group>
                                  ...
                                  <b-list-group-item>Nombre: {{ form.nombre }}
                            ...
                            <b-row>
                              <b-col lg="6">
                                ...
                                <!-- Opciones para sekeccuibar si es Custom o Sport -->
                                <!-- <b-form-radio-group
                                  id="btn-radios-2"
                            ...
                            <b-row>
                              <b-col lg="12" class="mt-4">
                                <b-table responsive :primary-key="form.productos.item" :fields="campos"
                                  :items="form.productos" small>
                                  ...
                                  <template #cell(cantidad)="data">
                                    <b-form-input v-model="form.productos[data.index].cantidad
                                      " min="0" :max="maxDesignLimit(data.item.cod)" type="number"
                                      @change="montoTotalOrden"></b-form-input>
                                  </template>

                                  <template #cell(corte)="data">
                                    <b-form-select :disabled="checkDesignForDiseabled(data.item.cod)
                                    ...
                                    <div>
                                      <span class="floatme">
                                        <b-button :disabled="checkDesignForDiseabled(
                                          ...
                                          " variant="primary" icon="ti-check" @click="
                              ...
                              <b-col lg="12">
                                ...
                                <quill-editor v-model="form.obs" @change="onEditorChange($event)"></quill-editor>
                              </b-col>
                            </b-row>
                          </div>
                        </b-tab>
                              ...
                              <b-col>
                                <div id="reporte">
                                  <b-overlay :show="overlay">
                                    <ordenes-presupuestoPreview :form="form" :showpreview="true" />
                                  </b-overlay>
                                </div>
                              </b-col>
                            </b-row>
...
export default {
  data() {
    return {
      endpoint: '/presupuesto/nuevo', // Opciones: `/presupuesto/nuevo` - `/presupuesto/nuevo`
  ...
  methods: {
    ...
    async getOrdenesGuardadas() {
      await axios.get(`${this.$config.API}/ordenes/guardadas`).then((res) => {
    ...
    async postPresupuesto() {
      this.form.obs = this.form.obs.replace('"', '\"')
    ...
    changeCategory() {
      ...
      if (this.categoriaDeLaORden === 'custom') {
        this.endpoint = `/presupuesto/nuevo`
    ...
    step3() {
      ...
      this.$confirm('¿Desea emitir el presupuesto?', 'Emitir', 'success').then(
        () => {
          this.overlay = true
          this.disableButtons = true
          this.finishOrder().then(() => {
            this.postPresupuesto().then(() => {
              this.$confirm(
 ...
 */
    ...
    async finishOrder() {
      ...
      data.set('montoDolaresPanamaDetalle', this.form.montoDolaresPanamaDetalle)
      data.set('montoPesosEfectivo', this.form.montoPesosEfectivo)
      data.set('montoPesosEfectivoDetalle', this.form.montoPesosEfectivoDetalle)
      ...
      await axios
        ...
        .then((data) => {
          console.log(`El presupuesto ${data.orden_nro} ha sido creado`)
          console.dir(data)

          this.$confirm('¿Desea imprimir el presupuesto?').then(() => {
            this.printOrder('reporte').then(() => {
              this.clearForm({
                form: true,
                formPrint: true,
        ...
        .catch((error) => {
          this.$fire({
            title: 'Error',
            html: `<p>Ocurrió un error al crear el presupuesto: ${error}</p>`,
            type: 'warning',
          })
        })

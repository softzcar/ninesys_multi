<template>
    <div>
        <b-button variant="warning" @click="$bvModal.show(modal)" title="Clonar Insumo" class="btn-clone text-dark">
                    ...
                    <b-row>
                        <b-col>
                            <b-form @submit="onSubmit" @reset="onReset">
                                ...
                                <h6 class="text-primary border-bottom pb-2 mb-3">1. Identificación (Clonando)</h6>
                                ...
                                <b-row v-if="form.tipo_insumo === 'tela'">
                                    <b-col>
                                        <b-form-group label="Modo de ingreso de costo:">
                                            ...
                                            ]" name="costo-mode-clone" buttons button-variant="outline-primary"
                                ...
                                <b-form-group v-if="costoInputMode === 'metro' && form.tipo_insumo === 'tela'"
                                    label="Precio por Metro:" label-for="input-costo-metro-clone">
                                    <campo-decimal id="input-costo-metro-clone" v-model="costoMetro" placeholder="0.00"></campo-decimal>
                                </b-form-group>

                                <b-form-group id="input-group-3-rend" label="Rendimiento (Mts/Kg):"
                                    label-for="input-rendimiento">
                                    <campo-decimal id="input-rendimiento" v-model="form.rendimiento" placeholder="0.00"></campo-decimal>
                                </b-form-group>
                                ...
                                <b-form-group id="input-group-3-cant" label="Cantidad (Kg, Mts, Ml, etc): *"
                                    ...
                                    <b-form-input id="input-cantidad" v-model="form.cantidad"
                                        placeholder="Ingrese la cantidad (Obligatorio)" required></b-form-input>
                                </b-form-group>
                                ...
                                <b-form-group id="input-group-4-costo"
                                    ...
                                    <campo-decimal id="input-costo" v-model="form.costo"
                                        ...
                                        placeholder="0.00 (Obligatorio)" required></campo-decimal>
                                </b-form-group>
...
export default {
            ...
            handler(newData) {
                if (newData) {
                    this.form = {
                        ...
                        cantidad: "", // MANDATORIO: Iniciar vacío
                        rendimiento: newData.rendimiento,
                        costo: "", // MANDATORIO: Iniciar vacío
    ...
    methods: {
        ...
        resetForm() {
            this.overlay = true
            this.form = {
                insumo: this.data ? this.data.insumo : "",
                sku: this.data ? this.data.sku : "",
                unidad: this.data ? this.data.unidad : "",
                cantidad: "",
                rendimiento: this.data ? this.data.rendimiento : 0,
                costo: "",
                departamento: this.data ? this.data.departamento : "",
                tipo_insumo: this.data ? (this.data.tipo_insumo || "general") : "general",
            }
            this.costoMetro = "";
            this.costoInputMode = "total";
            this.overlay = false
        ...
        async guardarInsumo() {
            ...
            data.set("cantidad_inicial", this.form.cantidad); // Blindaje de capacidad inicial
            ...
            try {
                ...
                if (response.data && !response.data.error) {
                    this.$fire({
                        title: "Insumo Clonado",
                        html: `<p>El insumo clonado <strong>${this.form.insumo}</strong> ha sido guardado exitosamente.</p>`,
                        type: "success",
                    });
                    this.resetForm();
                ...
                console.error("Error al clonar el insumo:", error);
                this.$fire({

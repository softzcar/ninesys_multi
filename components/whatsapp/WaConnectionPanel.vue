  ...
  <div>
    ...
    <b-card class="mb-4 border-0 shadow-sm" header="Acciones">
      <div class="d-flex flex-wrap gap-2">
        <b-button variant="outline-info" :disabled="actionLoading !== null" @click="doRestart">
        ...
        <b-button variant="outline-secondary" :disabled="actionLoading !== null" @click="refreshStatus">
    ...
    <b-card class="mb-4 border-0 shadow-sm">
      <div class="d-flex justify-content-between align-items-start">
        <div>
          ...
          <p class="text-muted mb-0">Sesión de WhatsApp vinculada a esta empresa.</p>
        </div>
    ...
    <!-- Código QR -->
    <b-card v-if="showQrSection" class="mb-4 border-0 shadow-sm" header="Vincular dispositivo">
    ...
    <b-card v-if="showHealth" class="mb-4 border-0 shadow-sm" header="Salud del Servicio">
...
export default {
  ...
  computed: {
    ...
    statusVariant() {
      const map = {
        READY: 'success', AUTHENTICATED: 'success', INITIALIZING: 'info',
        REQUIRES_QR: 'warning', PAUSED: 'secondary', DISCONNECTED: 'danger',
        ERROR: 'danger', DEGRADED: 'danger', NOT_REGISTERED: 'secondary',
      }
    ...
    statusLabel() {
      const map = {
        READY: 'Conectado', AUTHENTICATED: 'Autenticado', INITIALIZING: 'Inicializando...',
        REQUIRES_QR: 'Esperando QR', PAUSED: 'Pausado', DISCONNECTED: 'Desconectado',
        ERROR: 'Error', DEGRADED: 'Degradado', NOT_REGISTERED: 'Sin registrar',
      }
      return map[this.session.status] || this.session.status
    },

    showQrSection() {
      return ['NOT_REGISTERED', 'INITIALIZING', 'REQUIRES_QR'].includes(this.session.status)
    },
  ...
  watch: {
    'session.status'(val) {
      this.$emit('status-change', { status: val, variant: this.statusVariant })
    },
  },
  ...
  methods: {
    async fetchSessionInfo() {
      try {
        const { data } = await this.$wsApi.get(`/session-info/${this.idEmpresa}`)
        this.applySessionData(data)
      } catch (e) {
        console.error('[WaConnectionPanel] fetchSessionInfo error:', e.message)
      }
    },
    ...
    async fetchServiceHealth() {
      try {
        ...
        console.error('[WaConnectionPanel] fetchServiceHealth error:', e.message)
        this.serviceHealth = null
      }
    },

    applySessionData(data) {
      if (data.ws_ready && !data.status) {
        this.session.status = 'READY'
      } else {
        this.session.status = data.status || 'NOT_REGISTERED'
      }
      this.session.phone_number = data.phone_number || data.phoneNumber || data.info?.number || null
      this.session.pushname = data.pushname || data.info?.pushname || null
      this.session.last_seen_at = data.last_seen_at || null
      this.session.last_error = data.last_error || data.lastError || null
    ...
    async confirmDisconnect() {
      const ok = await this.$bvModal.msgBoxConfirm(
        ...
        { title: 'Desvincular sesión de WhatsApp', okVariant: 'danger', okTitle: 'Desvincular', cancelTitle: 'Cancelar' }
      )
      if (!ok) return
      ...
      try {
        ...
        this.$bvToast.toast('Sesión desvinculada. Generando nuevo código QR...', {
        ...
        this.$bvToast.toast(e.response?.data?.message || e.message, {
          title: 'Error al desvincular', variant: 'danger',
        })
      } finally {
        this.actionLoading = null
      }
    ...
    async refreshStatus() {
      ...
      try {
        const tasks = [this.fetchSessionInfo()]
        if (this.showHealth) tasks.push(this.fetchServiceHealth())
        await Promise.all(tasks)
        this.$bvToast.toast('Estado actualizado.', {
          title: 'Conexión WhatsApp', variant: 'info', autoHideDelay: 1500,
        })
      } finally {
        this.actionLoading = null
      }

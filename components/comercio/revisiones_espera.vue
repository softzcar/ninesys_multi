<template>
  <div>
    <b-overlay :show="overlay">
      <b-button :disabled="buttonDisabled" :variant="variants.button" @click="$bvModal.show(modal)">
        Revisiones Espera
      ...
      <b-modal :size="size" :title="title" :id="modal" hide-footer>
        ...
        <b-container>
          <b-row>
            <b-col>
              <span v-for="(rev, index) in revisionesActivas" :key="index" class="floatme">
                <b-button variant="outline-primary" :href="'#'" @click="smoothScroll(rev.id_orden)">{{ rev.id_orden
                  }}</b-button>
              </span>
            </b-col>
          </b-row>
          ...
          <b-row>
            <b-col>
              <b-card-group v-for="(rev, index) in revisionesActivas" v-bind:key="index" class="mb-4 mt-4" deck>

            ...
            <b-overlay :show="overlay" spinner-small>
                ...
                <div v-if="obj.length">
                    <h3 class="mb-4 mt-2">
                        <b-badge
                            :variant="variant"
                        >LA ORDEN {{ textList.idOrden }} ESTÁ {{ textList.status }}</b-badge>
                    </h3>
                    ...
                    <b-list-group>
                        <b-list-group-item :variant="variant">
                            <strong>Entrega Entrega estimada:</strong>
                    ...
                    <b-table-lite
                        ...
                        small
                        striped
                        :items="obj[0].items_departamentos"
                        :fields="fieldsDepartamentos"
                    >
                        <template #cell(fecha_inicio_calculada_depto_formateada)="data">
                            <!-- {{ data.item }} -->
                            {{ data.item.fecha_inicio_calculada_depto_formateada }} >>> {{ data.item.fecha_finalizacion_estimada_depto_formateada }}
                            <!-- {{ calcularProyeccionEmpleado(data.item.fecha_inicio_calculada_depto_formateada, ordenesTodas, textList.ordenFilaOrden, data.item.orden_proceso) }} -->
                            <!-- {{ calcularProyeccionEmpleado(data.item.fecha_inicio_calculada_depto_formateada, ordenesTodas, textList.ordenFilaOrden, data.item.orden_proceso) }} -->
                        </template>
                    </b-table-lite>
                </div>
                ...
                <div v-else>
                    <b-alert show>La orden aún no tiene personal adigando</b-alert>
                </div>

                <pre class="force">
                    <!-- {{textList.resumenTiempos}} -->
                     TODAS
                     <!-- {{ ordenesTodas }} -->
                     <hr />
                     OBJ
...
export default {
        ...
        return {
            ...
            fieldsDepartamentos: [
                ...
                },
                {
                    ...
                    label: "Fecha Terminar Calculada",
                },
                /* {
                    key: "fecha_finalizacion_estimada_depto_formateada",
                    label: "Estimado Finalización Antiguo",
                }, */
            ],
        };
    },
    ...
    computed: {
        ...
        title() {
            if (this.textList.idOrden === null) {
                return `Orden sin asignaciones`
            } else {
                return `Orden ${this.textList.idOrden}`
            }
        },
    },
    ...
    methods: {
        ...
        parseDatabaseDateTimeString(dateString) {
            ...
            if (!match) {
                console.error('Error al parsear fecha de la base de datos: El formato no es el esperado (YYYY-MM-DD HH:MM:SS).', dateString);
                return null;
            }
        ...
        calcularProyeccionEmpleado(fechaInicialString, procesosOrden, filaOrdenFila, ordenProceso) {
            ...
            if (fechaInicialString === null || typeof fechaInicialString === 'undefined') {
                console.error('Error: El string de fecha inicial es null o undefined.', fechaInicialString);
                return null;
            }
            ...
            if (!match) {
                console.error('Error: El formato de fecha inicial no es el esperado (DD/MM/YYYY HH:MM AM/PM).', fechaInicialString);
                return null;
            }
            ...
            if (isNaN(fechaActualProceso.getTime())) {
                console.error('Error: La fecha inicial proporcionada es inválida después de parsear.', fechaInicialString);
                return null;
            }

            // --- 2. Validar el Array de Procesos y el ID del Empleado ---
            if (!Array.isArray(procesosOrden) || procesosOrden.length === 0) {
                console.warn('Advertencia: El array de procesos de orden está vacío o no es válido.');
                return fechaInicialString; // Si no hay procesos, la fecha final es la inicial
            }

            if (typeof filaOrdenFila !== 'string' && typeof filaOrdenFila !== 'number') {
                console.error('Error: El filaOrdenFila debe ser un número o string numérico.');
                return null;
            }
            ...
            const tareasDelEmpleado = procesosOrden.filter(item => item.fecha_terminado === null && parseInt(item.orden_proceso_departamento) === ordenProceso && parseInt(item.orden_fila_orden) === parseInt(filaOrdenFila));
            console.log(`'tareasDelEmpleado' id orden ${this.textList.idOrden}`, tareasDelEmpleado);


            if (tareasDelEmpleado.length === 0) {
                console.warn(`Advertencia: El empleado con ID ${filaOrdenFila} no tiene tareas asignadas en los datos proporcionados.`);
            ...
            for (const item of tareasDelEmpleado) {
                ...
                if (isNaN(tiempoEstimado) || isNaN(cantidad)) {
                    console.warn(`Advertencia: 'tiempo_estimado_produccion' o 'cantidad' no es un número válido para el ítem:`, item);
                ...
                if (item.fecha_inicio !== null && item.fecha_terminado !== null) {
                    const fechaTerminadoTarea = this.parseDatabaseDateTimeString(item.fecha_terminado);
                    if (fechaTerminadoTarea && !isNaN(fechaTerminadoTarea.getTime())) {
                        ...
                        fechaActualProceso = new Date(Math.max(fechaActualProceso.getTime(), fechaTerminadoTarea.getTime()));
                        // No se suma tiempo, ya que la tarea está completada.
                    } else {
                        console.warn(`Advertencia: 'fecha_terminado' no pudo ser parseada para la tarea completada:`, item);
                    }
                }
                ...
                // Caso 3: Tarea No Iniciada (fecha_inicio == null y fecha_terminado == null)
                else { // Esto cubre ambos casos: tarea en curso o no iniciada
                    ...
                    if (item.fecha_inicio !== null) {
                        const fechaInicioTarea = this.parseDatabaseDateTimeString(item.fecha_inicio);
                        if (fechaInicioTarea && !isNaN(fechaInicioTarea.getTime())) {
                            ...
                            fechaActualProceso = new Date(Math.max(fechaActualProceso.getTime(), fechaInicioTarea.getTime()));
                        } else {
                            console.warn(`Advertencia: 'fecha_inicio' no pudo ser parseada para la tarea en curso/no iniciada:`, item);
                        }
                    }
                    ...
                    fechaActualProceso.setSeconds(fechaActualProceso.getSeconds() + segundosDeEstaTarea);
                }
            }

            // --- 5. Formatear la Nueva Fecha y Retornar ---
            const nuevoDia = String(fechaActualProceso.getDate()).padStart(2, '0');
            const nuevoMes = String(fechaActualProceso.getMonth() + 1).padStart(2, '0');
            const nuevoAnio = fechaActualProceso.getFullYear();
            let nuevasHoras = fechaActualProceso.getHours();
            const nuevosMinutos = String(fechaActualProceso.getMinutes()).padStart(2, '0');
        ...
        filterFechaEstimada() {
            ...
            this.textList.fechaEntrega =
                this.obj[0].fecha_entrega_formateada
            ...
            this.textList.tiempoEstimado =
                this.obj[0].tiempo_neto_orden_formateado;
    ...
    props: ["obj", "ordenesTodas", "ordenesProyectadas"],
};
</script>

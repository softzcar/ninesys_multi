<template>
  <div class="container mt-4">
    <b-card>
      ...
      <b-card-body>
        <b-button @click="enviarMensajeAlServidor" variant="primary"
          >Enviar Mensaje</b-button
        >
        <pre> wsResponse:{{ wsResponse }}</pre>
        <pre> formattedResponse:{{ formattedResponse }}</pre>
        <pre> $socket:{{ $socket.saludo() }}</pre>
        <b-alert :variant="alertVariant" show dismissible>
          {{ msgMounted }}
        </b-alert>
      </b-card-body>
    </b-card>

          ...
          <b-row>
            ...
            <!-- <b-col xl="6" lg="6" md="6" sm="12">
              <div class="float-button">
                <b-form-file v-model="newImage" :state="Boolean(newImage)" placeholder="Seleccione el diseño"
                  drop-placeholder="Arrastre el diseño aquí" accept="image/jpeg, image/png"></b-form-file>
                <div class="mt-3">
...
export default {
  ...
  methods: {
    async getImagen() {
      const url = `${this.$config.CDN}/disenos/imagen-aprovada${this.id}`
      await fetch(url)
        .then((res) => res.json())
        .then((res) => {
          // let rnd = Math.floor( Math.random() * 10 ) // PREVENIR CAHCHEAR LA IMAGEN
          this.imageUrl = `http://` + res.url + `?t=${this.token()}`
        })
        .catch((err) => {
          console.log(err)
        })

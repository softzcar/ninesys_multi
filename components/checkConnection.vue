<template>
    <div>
        ...
        <div v-if="$store.state.login.dataUser.acceso" class="floatme">
            <b-button @click="$bvModal.show(modalId)" variant="light">
                    ...
                    <b-row>
                        <b-col md="6">
                            ...
                            <ol>
                                <li>Abre WhatsApp.</li>
                                <li>
                                    Ve a <strong>Menú</strong> (<b-icon
...
export default {
    ...
    methods: {
        async getWSInfo() {
            ...
            console.log(
                `Fetching WS info for company ID: ${this.getCompanyId}`
            );
            try {
                ...
                console.error(
                    "Error al obtener información de WhatsApp:",
                    error
                );
                ...
                if (error.response) {
                    ...
                    this.ws = {
                        ...
                        details:
                            "Verifique la URL de la API y el estado del servidor.",
                    };
                } else {
                    console.error(
        ...
        restartPolling() {
            ...
            console.log(
                `Reiniciando polling con intervalo de ${
                    interval / 1000
                } segundos.`
            );
        ...
        async activateWhatsapp(companyId) {
            console.log(
                `Attempting to activate/get session info for ${companyId}`
            );
            ...
            try {
                ...
                console.error(
                    "Error during activation/session info fetch:",
                    error
                );
        ...
        async fetchQRCode(companyId) {
            ...
            try {
                ...
                if (this.ws.qr) {
                    ...
                    console.warn(
                        "API returned an error instead of QR:",
                        this.ws.error
                    );
                    this.statusWs.variant = "danger";
                } else {
                    console.warn(
                        "API returned no QR and no error. Status unknown."
                    );
                    this.ws.error =
                        res.data.message || "No se recibió QR ni error.";
                    this.statusWs.variant = "danger";
                }
            } catch (error) {
                console.error(
                    `Error fetching QR code for ${companyId}:`,
                    error
                );
                this.statusWs.variant = "danger";
                if (error.response) {
                    this.ws = {
                        ...
                        error:
                            error.response.data.message ||
                            "Error del servidor al obtener QR",
                        details: `Código: ${error.response.status}. ${
                            error.response.data.error || ""
                        }`.trim(),
                    };
        ...
        async performRestartClient(companyId) {
            ...
            try {
                ...
                console.error(
                    `Error performing restart for ${companyId}:`,
                    err
                );
                this.statusWs.variant = "danger";
                if (err.response) {
                    this.ws = {
                        ...
                        error:
                            ...
                            "Error del servidor al reiniciar",
        ...
        async performDisconnectClient(companyId) {
            ...
            try {
                ...
                console.error(
                    `Error performing disconnect for ${companyId}:`,
                    err
                );
                this.statusWs.variant = "danger";
                if (err.response) {
                    this.ws = {
                        ...
                        error:
                            err.response.data.message ||
                            "Error del servidor al desconectar",
                        details: `Código: ${err.response.status}. ${
                            err.response.data.error || ""
                        }`.trim(),
                    };

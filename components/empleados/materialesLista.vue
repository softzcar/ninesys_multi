...
export default {
  ...
  computed: {
    processedMaterialData() {
      ...
      console.log("Form para computed:", JSON.stringify(localForm, null, 2));

      if (!localDatainsumos || localDatainsumos.length === 0) {
        console.log("processedMaterialData: datainsumos vacío. Retornando [].");
        return [];
      }
      ...
      const agrupadosEstimado = localDatainsumos.reduce((acc, item) => {
        ...
        acc[idCatalogo].total_consumo_estimado += isNaN(consumoEstimadoActual)
          ? 0
      ...
      const resultadoFinal = Object.values(agrupadosEstimado).map(
        (itemEstimado) => {
          ...
          const matchingFormItems = safeForm.filter(
            (f) => f.idCatalogo === itemEstimado.id_catalogo_insumos_productos
          );
            ...
            consumoReal = matchingFormItems.reduce(
              (sum, currentFormItem) => {
                ...
                if (
                  ...
                  const inputComoString = String(currentFormItem.input).trim();
                  if (inputComoString !== "") {
                    const parsedInput = parseFloat(inputComoString);
          ...
          console.log(`  >>> consumoReal final para este item: ${consumoReal}`);

          let eficienciaPorcentaje = 0;
          const totalConsumoEstimado = itemEstimado.total_consumo_estimado;

          if (totalConsumoEstimado > 0) {
            eficienciaPorcentaje = (consumoReal / totalConsumoEstimado) * 100;
          ...
          const eficienciaFormateada = eficienciaPorcentaje.toFixed(2);
          ...
          return {
            ...
            _consumo_real_ingresado: parseFloat(consumoReal.toFixed(2)),
          };
        }
      );
      ...
      const total = this.datainsumos
        ...
        .reduce((acumulador, objeto) => {
          return (
            ...
            parseFloat(objeto.cantidad_estimada_de_consumo * objeto.unidades)
          );
        }, 0);

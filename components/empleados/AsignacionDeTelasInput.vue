  ...
  <div>
    <b-overlay class="floatme" :show="overlay" spinner-small>
      ...
      <div class="my-private-input floatme">
        <b-form-input type="number" step="0.1" v-model="value"></b-form-input>
      </div>
      <div class="floatme ml-2">
        <b-button class="floatme" variant="success" @click="updateCantidad"
...
export default {
  ...
  async postOrdenes() {
    ...
    await axios
    .post(
          `${this.$config.API}/inventario-movimientos/update-insumo`,
          data
          )
    .then((res) => {
      this.overlay = false

    ...
    <b-overlay :show="overlay" spinner-small>
      ...
      <div v-else>
        <b-container fluid>
          ...
          <b-row>
            <b-col>
              <h3>Reposiciones {{ dataTableReposiciones.length }}</h3>
            </b-col>
                  ...
                  <div style="width: 164%; float: left">
                    ...
                    <span class="floatme">
                      <b-button
                        ...
                        size="xl"
                        variant="info"
                        @click="iniciarTodo(row.item.orden)"
                        >Iniciar Todo
                      </b-button>
                    </span>
                ...
                <template #row-details="row">
                  <b-card>
                    <b-table
                      ...
                      stacked
                      responsive
                      :items="filterOrder(row.item.orden, 'por iniciar')"
                      :fields="fields"
                    >
                      <template #cell(departamento)="row">
                        {{ row.item.departamento }}
...
export default {
  ...
  computed: {
    ...
    insumosEstampado() {
      let options = this.insumos.filter(
        (item) =>
          item.departamento === "Estampado" || item.departamento === "Corte"
      );
      options.concat({ value: 0, text: "Seleccion insumo" });
      return options;
    },
  ...
  methods: {
    ...
    async getOrdenesAsignadas() {
      await axios
        .get(`${this.$config.API}/empleados/ordenes-asignadas/v2/${this.emp}`)
        .then((resp) => {
  ...
  fetch() {
    ...
    if (this.$store.state.login.dataUser.departamento === "Impresión") {
      ...
      // Cargar Insumos
    } else if (this.$store.state.login.dataUser.departamento === "Estampado") {

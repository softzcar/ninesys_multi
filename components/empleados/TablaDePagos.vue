  ...
  <div>
    ...
    <b-overlay :show="overlay" spinner-small>
      ...
      <b-list-group class="mb-4">
        <b-list-group-item>
          <h3 v-if="departamento != 'Comercialización'">RELACIÓN DE PAGOS</h3>
          <!-- <pre>{{ ordenes }}</pre> -->
        </b-list-group-item>
          ...
          >
            ...
            >
              ...
              <template #cell(calculo_pago)="data">
                ${{ parseFloat(data.item.calculo_pago).toFixed(2) }}
              </template>
            </b-table-lite>
          </b-tab>
...
export default {
    ...
    totalPendiente() {
      ...
      if (this.departamento === 'Diseño') {
        ...
        const ordenesCalcular = this.ordenesSemana.filter(
          (el) => el.progreso != 'terminada' && parseInt(el.cantidad) > 0
        )
  ...
  methods: {
    trabajosTerminados() {
      if (this.departamento === 'Diseño') {
        return this.ordenesSemana.filter((el) => el.estatus === 'Aprobado')
      } else if (
        this.departamento === 'Comercialización' ||
        this.departamento === 'Administración'
      ) {

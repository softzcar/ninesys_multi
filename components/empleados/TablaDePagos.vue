...
export default {
    ...
    computed: {
        ...
        totalHorasTrabajadas() {
            ...
            if (
                ...
                this.ordenesTerminadas.length === 0
            ) {
    ...
    methods: {
        tiempoTranscurridoEnMinutos(fecha_inicio, fecha_terminado) {
            ...
            const diferenciaMs =
                fechaTerminado.getTime() - fechaInicio.getTime();
        ...
        async getOrdenesAsignadas() {
            await this.$axios
                .get(
                    `${this.$config.API}/reportes/resumen/empleados/${this.emp}/${this.$store.state.login.currentDepartamentId}`
                )
                .then((resp) => {

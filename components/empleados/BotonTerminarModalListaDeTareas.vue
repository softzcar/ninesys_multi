    ...
    <b-modal :id="modal" :title="title" hide-footer size="xl">
      ...
      <b-overlay :show="overlay" spinner-small>
        ...
        " show variant="danger">
          ...
          <p>
            Seleccione los rollos de tela que utilizó antes de terminar la
            tarea.
          </p>
        </b-alert>
        ...
        <div v-else>
          <div v-if="this.$store.state.login.dataUser.departamento === 'Corte'">
            ...
            <b-table class="mb-4" :fields="fields" striped :items="dataTable2">
              <template #cell(cantidad)="data">
                <empleados-BotonTerminarModalInput :item="data.item" :itemfather="itemfather" :index="data.index"
                  @reload="reloadMe" />
              </template>
            </b-table>
          </div>
          ...
          <div v-else>
            <h3 class="text-center">Va a terminar la terea en curso</h3>
          </div>

          <div class="text-center mt-4 mb-4">
            <b-button :variant="ButtonVariant" @click="terminarTrabajo(item)">{{
              ButtonText
            }}</b-button>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    if (this.item.fecha_inicio === null) {

            ...
            <b-modal :id="modal" title="WhatsApp" hide-footer size="xl">
                ...
                <b-container v-else>
                    <b-row>
                        <b-col>
                            <h2 class="mt-2 mb-4">
                                Use Whatsapp para eviar mensajes a sus clientes
                            </h2>
                            <ol>
                                <li>Abra WhatsApp en el su teléfono</li>
                                <li>
                                    Toque <strong>Menú</strong>
                                ...
                                <li>
                                    Toque el botón
                                    <strong>Vincular in dispositivo</strong>
                                </li>
...
export default {
    ...
    methods: {
        ...
        async getWSInfo() {
            try {
                const res = await this.$axios.get(
                    `${this.$config.API}/ws-connect/${this.$store.state.login.dataEmpresa.id}`
                );
                this.ws = res.data;
                this.statusWs.variant = this.ws.ws_ready ? "success" : "danger";
            } catch (error) {
        ...
        restartPolling() {
            ...
            const interval = this.ws.ws_ready
                ? this.longInterval
                : this.shortInterval;

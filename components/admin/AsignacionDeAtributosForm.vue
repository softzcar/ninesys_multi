    ...
    <div>
        <b-overlay :show="overlay" spinner-small>
            <div class="mb-2" v-if="asignado">
                <b-alert variant="info" show>Atributo asignado {{ attributeText }}</b-alert>
            </div>
            <b-form inline>
                <b-form-group id="input-group-1" label="" label-for="select-atributo"
                    description="Seleccione un atributo.">
                    <b-form-select id="select-atributo" :disabled="inputDisabled" v-model="attribute" :options="options"
                        :value="attribute" class="floatme"></b-form-select>
                    <b-button @click="showPrompt" id="add-atributo" variant="light">
                        <b-icon icon="plus-lg"></b-icon>
                    </b-button>
                    <b-popover target="add-atributo" triggers="hover" placement="top">
                        <template #title>Crear nuevo atributo</template>
                    </b-popover>
                </b-form-group>
            </b-form>
            <b-form class="mt-2">
                <b-form-group id="input-group-2" label="Valor del atributo" label-for="input-descripción"
                    description="Indique el valor del atributo.">
                    <b-form-input id="input-descripción" type="text" v-model="attributeText"
    ...
    methods: {
        async showPrompt() {
            try {
                ...
                const input = await this.$prompt("Por favor, escribe tu nombre:");
                if (input && input.trim() !== "") {
        ...
        saveChange() {
            ...
            if (ok) {
                ...
                alert('todo OK vamos a guardar el registro en el formularieo para enviarlo posteriormente')
            } else {
                this.$fire({

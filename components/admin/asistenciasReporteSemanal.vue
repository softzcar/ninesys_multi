      ...
      <div v-for="employee in processedData" :key="employee.nombre">
        <b-row>
          <b-col>
            <h3 style="text-transform: capitalize">
              ...
              <small>
                <strong
                  >{{ sumarHoras(totalHoras(employee.nombre)) }} Horas</strong
                >
        ...
        <b-row class="mb-4">
          <b-col class="col-calendario">
            ...
            >
              {{ formatRegistro(emp.registro) }}: {{ convertHour(emp.hora) }}
            </div>
          </b-col>
        ...
        tmpDia = dataEmpleado
          ...
          .map((el, index, arr) => {
            ...
            if (el.registro === 'Entrada mañana') {
              ...
              if (respManana === undefined || respManana === null) {
                ...
                if (esHoy) {
                  ...
                  horaSalidaManana =
                    parseInt(tmpSplit[0]) - 1 + ':' + tmpSplit[1]
                } else {
              ...
              horaEntradaTarde = arr
                .filter(
                  (asistencia) =>
                    asistencia.registro === 'Entrada tarde' &&
                    asistencia.empleado === el.empleado &&
                    asistencia.fecha === el.fecha
                )
                .map((el) => {
              ...
              if (horaEntradaTarde.length > 0) {
                const respTarde = arr.find(
                  (asistencia) =>
                    asistencia.registro === 'Salida tarde' &&
                    asistencia.empleado === el.empleado &&
                    asistencia.fecha === el.fecha
                )

                if (respTarde === undefined || respTarde === null) {
                  ...
                  if (esHoy) {
                    horaSalidaTarde = this.obtenerHoraActual()
                  } else {
              ...
              // Sumar horas
              tot = parseFloat(horasManana) + parseFloat(horasTarde)
              /* console.log(

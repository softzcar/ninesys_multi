...
-   Eliminar el sitio msg.nineteengreen.com desde CyberPanel para crearlo de nuevo via ssh con el usuario `ozcar` para poder ejecutar `npm run start` y correr la aplicacion en el puerto `3000`.
-   En CloudFlare se configuro un registro A para el subdominio msg.ninetenngreen.com y un CNAME llamado wsm.nineteengreen.com por sugerencia de copilot para redireccionar el puerto 3000 Pero no se probó.
-   **Rescatar _.htaccess_ de el root antes de elimintar el sitio** porque puede ser util para redireccionar nuevamente al puerto 3000.
-   El puerto 3000 ya está abierto en el firewall, Quedamos en probar correr la aplicacón para verificar si funciona directametne, o si el servidor lightSpeed interfiere en esta tarea.

Queda la opción de probar instalar la imágen con docker que ya ha sido instalado y la imágen ha sido creada y es reconocida por CiberPanel como una imágen local, solo tener en cuenta que `para correr la plicacion no se puede ser usuario root` porque Chromium no lo permite. Tal vez configurando en Docker los puertos internos y extermos podamos llear a desplegar esa imágen siempte y cuando el usario que la desplegue no sea root; averiguar que onda con eso proque CyberPanel usa root para ejecutar los comandos, ssin embargo falta probar crear otro usuario nuevo y ver con que permisos lo crea o si se puedem modificar los permisos.

## MSG_NINE Quedamos

-   Se me ocurre probar instalar una interfaz gráfica en el VPN para probar si puppeter la reconoce sin estar ejecutandose para resolver el problema del envio de mdensaje

...
[ ] La consulta de la tabla del CRUD de empleados arreglar buscar nombre de Departamento (S) para mosntrar, buscar una solucuón a esto
[ ] Quitar etiquetas <pre></pre>
[ ] Los empleados se stan loguenado correctametne porque se esta usando la logica antigua y el campo `departamento` de la tabla de `empleados` que ya está en desuso, programar sistema de login atendiendo al departametneo y al modulo asignado en las tablas deempresa

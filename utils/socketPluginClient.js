function socketWebClient(server, app) {
  ...
  function init() {
    ...
    socket.onopen = function () {
      queue = []
      callbackStatus('Conectado')
    }
    socket.onerror = function () {
      if (socketSend === false) {
        callbackStatus('No se conectó al servidor especificado')
      }
      socketSend = false
      socketOpen = false
      queue = []
    ...
    socket.onmessage = function (msg) {
      ...
      if (packet.opcode === 'close') {
        ...
        socketOpen = false
        socketSend = false
        callbackStatus('La conexión con el servidor ha sido cerrada')
        return
      }
      //********************************************
  ...
  function setCallbackReadMessage(func) {
    //  overwrite dummy call back with your own func
    console.log('probando el callback de mensajes')

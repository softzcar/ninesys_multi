              ...
              <b-col>
                <b-alert show class="mb-4" variant="warning">
                  ...
                  <strong>Si va a hacer un retiro de efectivo para algun gasto,
                    ...
                    <router-link class="nav-link" to="/comercializacion/retiros" custom v-slot="{ navigate }">
                      <span @click="navigate" @keypress.enter="navigate" style="display: inline; padding: 0"
                        class="blue-link" role="link">
                        Retiros
                      </span>
                    </router-link></strong>
                </b-alert>
            ...
            <b-row>
              <b-col>
                <h2 class="mb-4">
                  ...
                  <b-button size="lg" class="ml-4" variant="success" @click="enviarCierre">RETIRAR</b-button>
                </h2>
              </b-col>
            </b-row>
            <b-row>
              <b-col>
                <H2>TABLA DE MONTOS EN CAJA POR TIPO DE MOENEDA</H2>
              </b-col>
            </b-row>
            ...
            <b-row>
              <b-col xl="4" lg="4" md="4" sm="12" xs="12">
                <b-row>
                  <b-col>
                    <hr />
                    <h4>Dólares Max {{ formatNumber(totDolares) }}</h4>
                  </b-col>
                </b-row>

                <b-row align-h="start">
                  <b-col>
                    <b-form-group id="input-group-1" label-for="input-dolares-efectivo" class="pl-2">
                      <b-form-input id="input-dolares-efectivo" type="number" step="0.10" min="0" :max="totDolares"
              ...
              <b-col xl="4" lg="4" md="4" sm="12" xs="12">
                <b-row>
                  <b-col>
                    <hr />
                    <h4>Pesos Max {{ formatNumber(totPesos) }}</h4>
                  </b-col>
                </b-row>

                <b-row align-h="start">
                  <b-col>
                    <b-form-group id="input-group-4" label-for="input-dolares-efectivo" class="pl-2">
                      <b-form-input id="input-pesos-efectivo" type="number" step="0.10" min="0" :max="totPesos"
              ...
              <b-col xl="4" lg="4" md="4" sm="12" xs="12">
                <b-row>
                  <b-col>
                    <hr />
                    <h4>Bolívares Max {{ formatNumber(totBolivares) }}</h4>
                  </b-col>
                </b-row>

                <b-row align-h="start">
                  <b-col>
                    <b-form-group id="input-group-6" label-for="input-bolivares-efectivo " class="pl-2">
                      <b-form-input id="input-bolivares-efectivo" type="number" step="0.10" min="0" :max="totBolivares"
...
export default {
  ...
  methods: {
      ...
      if (ok) {
        this.$confirm(
          ...
          .then(() => {
            ...
            axios
              .post(`${this.$config.API}/cierre-de-caja`, data)
              .then((res) => {
                this.form = {
                  detalle: '',
                  montoDolaresEfectivo: 0,
    ...
    async getDataCierre() {
      await axios
        .get(`${this.$config.API}/cierre-de-caja`)
        .then((res) => {
          this.efectivo = res.data.data.caja
          this.fondo = res.data.data.fondo

          if (this.efectivo[0].monto === null) {
            this.efectivo[0].moneda = 'Dólares'
            this.efectivo[0].tasa = '1'
            this.efectivo[0].dolares = '0'
            this.efectivo[0].monto = this.fondo[0].dolares
            this.totDolares = this.fondo[0].dolares
          } else {
            this.totDolares = this.efectivo[0].monto
          }

          if (this.efectivo[1].monto === null) {
            this.efectivo[1].moneda = 'Pesos'
            this.efectivo[1].tasa = this.peso
            this.efectivo[1].dolares = '0'
            this.efectivo[1].monto = this.fondo[0].pesos
            this.totPesos = this.fondo[0].pesos
          } else {
            this.totPesos = this.efectivo[1].monto
          }

          if (this.efectivo[2].monto === null) {
            this.efectivo[2].moneda = 'Bolívares'
            this.efectivo[2].tasa = this.dolar
            this.efectivo[2].dolares = '0'
            this.efectivo[2].monto = this.fondo[0].bolivares
            this.totBolivares = this.fondo[0].bolivares
          } else {
            this.totBolivares = this.totBolivares
          }

          console.log('efecrivo', this.efectivo)
        })
        .catch((err) => {

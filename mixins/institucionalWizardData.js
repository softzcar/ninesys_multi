...
// Datos y helpers compartidos por los pasos institucionales del wizard
// (Datos del Administrador, Datos de la Empresa, Monedas, Horario Laboral,
// Personalización, Gastos Fijos). Extraído de configuracionWizard.vue (que se
// deja intacto, sigue existiendo como wizard de comparación) para no
// triplicar esta lógica también en OperativaWizard.vue.
export default {
  ...
  methods: {
    loadInitialData() {
      ...
      if (personalizacionData) {
        ...
        // configuracionWizard.vue y pages/empresa/configuracion.vue).
        const LEGACY_FIELDS = [
      ...
      // Sincronizar Gastos Fijos con la API (para casos donde se modificaron
      // fuera del wizard, ej. desde la página dedicada de Gastos).
    ...
    async fetchLatestGastos() {
      try {
        ...
        console.error("Error al sincronizar gastos fijos:", error);
      }
    },

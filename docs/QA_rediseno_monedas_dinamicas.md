...
- [x] En **todos** los formularios anteriores: para un método marcado como "requiere referencia" (ej. Zelle, Pagomovil, Transferencia), el campo de referencia/detalle **es obligatorio** → *Esperado (cambiado 2026-08-03, ver `project_rediseno_monedas_metodos_pago.md`):* el campo aparece; si se deja vacío en un método que lo requiere y con monto > 0, el envío se bloquea con un aviso y el input queda resaltado en rojo -- ya NO se guarda el pago sin referencia. Validación centralizada en `MetodosPagoDinamico.vue`, verificada en los 6 formularios que lo usan.

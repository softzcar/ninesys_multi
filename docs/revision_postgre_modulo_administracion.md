...
- [x] **PENDIENTE anotado 2026-07-27, resuelto 2026-08-11 (commit `2911f0e`):** "Diseños Terminados" ya filtra por diseñador logueado + últimos 30 días (`GET /disenos/terminados?id_empleado&dias`); de paso se corrigieron un WHERE tautológico, un leak de SQL en la respuesta y una ruta legada muerta con path param. Ver [[project_auditoria_modulo_empleados_diseno]].
...
- [x] Historial de pagos **(`/empresa/gastos-historial`, componente `GastosHistorial.vue` sobre el mismo backend `/gastos/registros` ya auditado en Gastos Adicionales. Hallazgo real: el filtro de fecha por defecto era "mes actual", pero los 3 únicos pagos reales de la empresa son de julio -- la página se veía vacía ("No se encontraron pagos") al entrar, sin ninguna explicación, dando la falsa impresión de que no hay historial. Corregido quitando el rango por defecto: ahora carga el historial completo al entrar. De paso, `DELETE /gastos/registros/{id}` no envolvía en transacción el INSERT de auditoría + el DELETE real -- si el DELETE fallara después de auditar, quedaría un "eliminado" fantasma en la bitácora para un registro que seguía existiendo; corregido con el mismo patrón de transacción que ya tenía el PUT hermano. Verificado end-to-end con curl y un registro 100% desechable: crear → aparece en el listado → editar (auditoría "editado" con monto_anterior/nuevo correctos) → eliminar (auditoría "eliminado", registro realmente borrado) → los 3 registros reales nunca se tocaron. Los 2 registros de auditoría generados por la prueba se limpiaron de la BD al terminar)**
- [x] Bitácora de gastos **(`/empresa/gastos-bitacora`, componente `GastosAuditoria.vue` sobre `GET /gastos/auditoria`, ya parametrizado desde el principio. Mismo bug de filtro por defecto que Historial de Pagos (antes "últimos 7 días", corregido a mostrar todo el historial al entrar) -- con 0 acciones reales registradas hasta hoy el síntoma no era visible aún, pero se hubiera repetido apenas existiera una edición/eliminación real más vieja que la ventana por defecto. Verificado con la misma prueba desechable de arriba: el "editado" y el "eliminado" aparecieron correctamente con usuario, motivo y montos, sin filtro alguno)**
